...
namespace SCNCore_Plus_Server.Microsoft_Defender_Firewall
{
    internal class Handler
    {
        ...
        public static void Rule_Inbound(string port)
        {
            try
            {
                ...
                bool rule_existing = firewallPolicy.Rules.Cast<INetFwRule>().Any(rule => rule.Name == "SCNCore Plus Server Inbound HTTP (" + port + ")");
                ...
                if (!rule_existing)
                {
                    ...
                    new_rule.Name = "SCNCore Plus Server Inbound HTTP (" + port + ")";
            ...
            {
                Logging.Handler.Error("Microsoft_Defender_Firewall.Rule_Inbound", "Add SCNCore service rule (outbound)", ex.ToString());
            }
        }
        ...
        public static void Rule_Outbound(string port)
        {
            try
            {
                ...
                // Check if SCNCore service rule is existing
                bool rule_existing = firewallPolicy.Rules.Cast<INetFwRule>().Any(rule => rule.Name == "SCNCore Plus Server Outbound HTTPS (" + port + ")");
                ...
                if (!rule_existing)
                {
                    ...
                    INetFwRule2 new_rule = (INetFwRule2)Activator.CreateInstance(Type.GetTypeFromProgID("HNetCfg.FWRule"));
                    new_rule.Name = "SCNCore Plus Server Outbound HTTPS (" + port + ")";
                    new_rule.Enabled = true;
            ...
            {
                Logging.Handler.Error("Microsoft_Defender_Firewall.Rule_Outbound", "Add SCNCore service rule (outbound)", ex.ToString());
            }
        }
    }
}
