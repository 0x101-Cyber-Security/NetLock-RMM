...
using SCNCore_Plus_Server.Agent.Windows;
using System.Security.Principal;
using Microsoft.AspNetCore.SignalR;
using SCNCore_Plus_Server.SignalR;
using System.Net;
using System;
using System.Text.Json;
using static SCNCore_Plus_Server.Agent.Windows.Authentification;
using Microsoft.Extensions.DependencyInjection;
using SCNCore_Plus_Server;
using SCNCore_Plus_Server.Events;
using Microsoft.Extensions.Primitives;
using LettuceEncrypt;
using System.Threading;
using System.IO;
using static SCNCore_Plus_Server.SignalR.CommandHub;
using Microsoft.AspNetCore.Builder;
using LLama.Common;
using LLama;
//using SCNCore_Plus_Server.LLM;
using Microsoft.AspNetCore.Mvc;
using System.Runtime.InteropServices;
using SCNCore_Plus_Server.Configuration;
using System.Security.Cryptography;
using SCNCore_Plus_Server.Members_Portal;
...
using SCNCore_Plus_Server.Background_Services;

SCNCore_Plus_Server.Configuration.Server.serverStartTime = DateTime.Now; // Set server start time

// Check directories
SCNCore_Plus_Server.Setup.Directories.Check_Directories(); // Check if directories exist and create them if not
...
// Members Portal Api
var membersPortal = builder.Configuration.GetSection("Members_Portal_Api").Get<SCNCore_Plus_Server.Members_Portal.Config>() ?? new SCNCore_Plus_Server.Members_Portal.Config();
...
// Output version
Console.WriteLine("SCNCore Plus Server");
...
var mysqlConfig = builder.Configuration.GetSection("MySQL").Get<SCNCore_Plus_Server.MySQL.Config>() ?? new SCNCore_Plus_Server.MySQL.Config();
...
bool microsoft_defender_firewall_status = SCNCore_Plus_Server.Microsoft_Defender_Firewall.Handler.Status();
...
else if (!microsoft_defender_firewall_status && OperatingSystem.IsWindows())
{
    ...
    Console.WriteLine("Microsoft Defender Firewall is disabled. You should enable it for your own safety. SCNCore adds firewall rules automatically according to your configuration.");
}

Console.ResetColor();

// Add firewall rule for HTTP
SCNCore_Plus_Server.Microsoft_Defender_Firewall.Handler.Rule_Inbound(http_port.ToString());
SCNCore_Plus_Server.Microsoft_Defender_Firewall.Handler.Rule_Outbound(http_port.ToString());

if (https)
{
    // Add firewall rule for HTTPS
    SCNCore_Plus_Server.Microsoft_Defender_Firewall.Handler.Rule_Inbound(https_port.ToString());
    SCNCore_Plus_Server.Microsoft_Defender_Firewall.Handler.Rule_Outbound(https_port.ToString());
...
if (!await SCNCore_Plus_Server.MySQL.Handler.Check_Connection())
{
...
else
{
    ...
    if (!await SCNCore_Plus_Server.MySQL.Handler.Verify_Supported_SQL_Server())
    {
    ...
    await SCNCore_Plus_Server.MySQL.Handler.Update_Server_Information();

    // Get api key
    if (String.IsNullOrEmpty(Members_Portal.api_key))
    {
        Members_Portal.api_key = await SCNCore_Plus_Server.MySQL.Handler.Get_Members_Portal_Api_Key();
...
if (role_comm)
{
    app.MapPost("/Agent/Windows/Check_Version", async context =>
    {
        try
        {
            ...
            {
                bool package_guid_status = await Verify_SCNCore_Package_Configurations_Guid(package_guid);
...
/*app.MapPost("/Agent/Windows/Remote/Command", async (HttpContext context, IHubContext<CommandHub> hubContext) =>
{
    try
    {
        ...
        bool api_key_status = await SCNCore_Plus_Server.SignalR.Webconsole.Handler.Verify_Api_Key(json);
        ...
        // Get the command
        string command = await SCNCore_Plus_Server.SignalR.Webconsole.Handler.Get_Command(json);
...
if (role_file)
{
    app.MapPost("/admin/files/index/{path}", async (HttpContext context, string path) =>
    {
        try
        {
            ...
            // Verify API key
            if (!context.Request.Headers.TryGetValue("x-api-key", out StringValues apiKey) || !await SCNCore_Plus_Server.Files.Handler.Verify_Api_Key(apiKey))
            {
                context.Response.StatusCode = 401;
                await context.Response.WriteAsync("Unauthorized.");
                return;
...
if (role_file)
{
    app.MapPost("/admin/files/command", async context =>
    {
        try
        {
            ...
            if (!hasApiKey || !await SCNCore_Plus_Server.Files.Handler.Verify_Api_Key(files_api_key))
            {
            ...
            await SCNCore_Plus_Server.Files.Handler.Command(json);
...
if (role_file)
{
    app.MapPost("/admin/files/upload/{path}", async (HttpContext context, string path) =>
    {
        try
        {
            ...
            bool ApiKeyValid = await SCNCore_Plus_Server.Files.Handler.Verify_Api_Key(files_api_key);
            ...
            string register_json = await SCNCore_Plus_Server.Files.Handler.Register_File(filePath, tenant_guid, location_guid, device_name);
...
if (role_file)
{
    app.MapGet("/admin/files/download", async (HttpContext context) =>
    {
        try
        {
            ...
            bool hasAccess = await SCNCore_Plus_Server.Files.Handler.Verify_File_Access(guid, password, files_api_key); // api key is not required
            ...
            string file_path = await SCNCore_Plus_Server.Files.Handler.Get_File_Path_By_GUID(guid);
...
// SCNCore admin files device download
app.MapGet("/admin/files/download/device", async (HttpContext context) =>
{
    try
    {
        ...
        if (device_status == "authorized" || device_status == "synced" || device_status == "not_synced")
        {
            ...
            bool file_access = await SCNCore_Plus_Server.Files.Handler.Verify_Device_File_Access(tenant_guid, location_guid, device_name, guid);
...
// SCNCore admin files device upload
...
// SCNCore files download private - GUID, used for update server & trust server
    ...
    app.MapPost("/llm/chat/admin", async (HttpContext context, LLaMaService llamaService) =>
    {
        try
        {
            ...
            if (!context.Request.Headers.TryGetValue("x-api-key", out var apiKey) ||
                !await SCNCore_Plus_Server.Files.Handler.Verify_Api_Key(apiKey))
            {
                context.Response.StatusCode = 401;
                await context.Response.WriteAsync("Unauthorized.");
                return;
...
if (role_file)
{
    app.MapPost("/admin/create_installer", async (HttpContext context) =>
    {
        try
        {
            ...
            string result = await SCNCore_Plus_Server.Files.Handler.Create_Custom_Installer(name, server_config);
