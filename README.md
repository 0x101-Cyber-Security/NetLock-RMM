...
**Security:** Please do not use AI tools or other methods to analyze the security of the code published in our GitHub repositories or report potential vulnerabilities based on it. Our closed source release has evolved significantly beyond the now outdated GitHub codebase, which represents an early prototype of NetLock RMM and no longer reflects the current product's architecture or security posture. If you're interested in security research or penetration testing, we welcome you to deploy NetLock RMM through our Members Portal on your own self hosted server and test it as extensively as you like. Please do not target or attack our cloud infrastructure. If you discover a vulnerability that could compromise a NetLock RMM server instance, such as remote code execution (RCE), authentication bypass, or similar issues, please contact us. We offer monetary rewards based on the severity of the vulnerability, as well as a **lifetime self-hosted NetLock RMM license with unlimited devices for one server**. We appreciate responsible security research and welcome reports of genuine vulnerabilities. Please send your findings to **[nico.mak@0x101-cyber-security.de](mailto:nico.mak@0x101-cyber-security.de)**.
