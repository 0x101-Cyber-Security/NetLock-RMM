...
using SCNCore_Plus_Web_Console.Components;
using SCNCore_Plus_Web_Console;
using System.Net;
using SCNCore_Plus_Web_Console.Classes.MySQL;
using Blazored.LocalStorage;
using Microsoft.AspNetCore.Components.Authorization;
using Microsoft.AspNetCore.Components.Server.ProtectedBrowserStorage;
using SCNCore_Plus_Web_Console.Classes.Authentication;
using System.Runtime.InteropServices;
using System.Runtime.Versioning;
using SCNCore_Plus_Web_Console.Configuration;
using Microsoft.AspNetCore.Hosting.Server;
using Microsoft.Extensions.Configuration;
using Microsoft.AspNetCore.Http.Features;
using SCNCore_Plus_Web_Console.Components.Pages.Devices;
using LettuceEncrypt;
using LettuceEncrypt.Acme;
using MudBlazor;
using Microsoft.AspNetCore.HttpOverrides;
using static MudBlazor.Defaults;
using SCNCore_Plus_Web_Console.Classes.Helper;
using Org.BouncyCastle.Asn1.X509.Qualified;

SCNCore_Plus_Web_Console.Classes.Setup.Directories.Check_Directories(); // Check if directories exist and create them if not
...
var remoteServerConfig = builder.Configuration.GetSection("SCNCore_Remote_Server").Get<SCNCore_Plus_Web_Console.Classes.Remote_Server.Config>();
...
var fileServerConfig = builder.Configuration.GetSection("SCNCore_File_Server").Get<SCNCore_Plus_Web_Console.Classes.File_Server.Config>();
...
// Title
Web_Console.title = builder.Configuration.GetValue<string>("Webinterface:Title", "SCNCore Plus");

if (Web_Console.title == "Your company name")
    Web_Console.title = "SCNCore Plus"; // Default title if not set
...
// Output version
Console.WriteLine("SCNCore Plus Web Console");
...
else
{
    ...
    else
    {
        ...
        else // Table exists
        {
            ...
            if (String.IsNullOrEmpty(Members_Portal.api_key))
            {
                Members_Portal.api_key = await SCNCore_Plus_Web_Console.Classes.MySQL.Handler.Get_Api_Key();

                Console.WriteLine("Members Portal API key loaded from database: " + Members_Portal.api_key);
            }
            else
                await SCNCore_Plus_Web_Console.Classes.Members_Portal.Handler.Set_Api_Key(Members_Portal.api_key);

            // Do cloud stuff
            if (Members_Portal.cloud_enabled)
            {
                Console.WriteLine("Cloud enabled. Checking cloud connection...");
                if (await SCNCore_Plus_Web_Console.Classes.Members_Portal.Handler.Check_Connection())
                {
                    Console.ForegroundColor = ConsoleColor.Green;
            ...
            // Update license info
            await SCNCore_Plus_Web_Console.Classes.Members_Portal.Handler.Request_License_Info_Json(Members_Portal.api_key);
        }
    }
}
...
builder.Services.AddHostedService<SCNCore_Plus_Web_Console.Classes.MySQL.AutoCleanupService>();
builder.Services.AddHostedService<SCNCore_Plus_Web_Console.Classes.ScreenRecorder.AutoCleanupService>(); //disabled until remote screen control release
