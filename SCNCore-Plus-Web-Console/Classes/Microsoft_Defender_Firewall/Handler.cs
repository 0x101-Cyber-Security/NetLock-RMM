    ...
    public class Microsoft_Defender_Firewall
    {
        ...
        public static void Rule_Inbound(string port)
        {
            try
            {
                ...
                bool rule_existing = firewallPolicy.Rules.Cast<INetFwRule>().Any(rule => rule.Name == "SCNCore Plus Web Console Inbound HTTP (" + port + ")");
                ...
                if (!rule_existing)
                {
                    ...
                    new_rule.Name = "SCNCore Plus Web Console Inbound HTTP (" + port + ")";
        ...
        public static void Rule_Outbound(string port)
        {
            try
            {
                ...
                // Check if SCNCore service rule is existing
                bool rule_existing = firewallPolicy.Rules.Cast<INetFwRule>().Any(rule => rule.Name == "SCNCore Plus Web Console Outbound HTTPS (" + port + ")");
                ...
                if (!rule_existing)
                {
                    ...
                    INetFwRule2 new_rule = (INetFwRule2)Activator.CreateInstance(Type.GetTypeFromProgID("HNetCfg.FWRule"));
                    new_rule.Name = "SCNCore Plus Web Console Outbound HTTPS (" + port + ")";
                    new_rule.Enabled = true;
