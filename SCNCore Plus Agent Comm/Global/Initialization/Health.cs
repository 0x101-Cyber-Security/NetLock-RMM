    ...
    internal class Health
    {
        ...
        { 
            try
            {
                ...
                if (!Windows.Helper.Registry.HKLM_Key_Exists(Application_Paths.scncore_reg_path))
                    Windows.Helper.Registry.HKLM_Create_Key(Application_Paths.scncore_reg_path);

                // Check if msdav key exists
                if (!Windows.Helper.Registry.HKLM_Key_Exists(Application_Paths.scncore_microsoft_defender_antivirus_reg_path))
                    Windows.Helper.Registry.HKLM_Create_Key(Application_Paths.scncore_microsoft_defender_antivirus_reg_path);
            }
            catch (Exception ex)
            {
        ...
        {
            Windows.Microsoft_Defender_Firewall.Handler.SCNCore_Plus_Comm_Agent_Rule_Inbound();
            Windows.Microsoft_Defender_Firewall.Handler.SCNCore_Plus_Comm_Agent_Rule_Outbound();
            Windows.Microsoft_Defender_Firewall.Handler.SCNCore_Plus_Health_Service_Rule();
            Windows.Microsoft_Defender_Firewall.Handler.SCNCore_Installer_Rule();
            Windows.Microsoft_Defender_Firewall.Handler.SCNCore_Uninstaller_Rule();
        }
        ...
        {
            // Check if the databases are in place
            if (!File.Exists(Application_Paths.program_data_scncore_policy_database))
                Database.SCNCore_Data_Setup();

            // Check if the events database is in place
            if (!File.Exists(Application_Paths.program_data_scncore_events_database))
                Database.SCNCore_Events_Setup();
        }
